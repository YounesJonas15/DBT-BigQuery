with
    source as (
        ...
        left join {{ ref("dim_customer") }} as c on po.customer_id = c.customer_id
        left join {{ ref("dim_employee") }} as e on e.employee_id = po.employee_id
        left join {{ ref("dim_product") }} as p on po.product_id = p.product_id
    )
select *
from source
